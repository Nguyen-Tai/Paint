    ...
    class Circle:Ellipse
    {
        protected override GraphicsPath GraphicsPathObject
        {
            get
            {
                ...
                if (this.p2.X < this.p1.X && this.p2.Y < this.p1.Y) // lên trái
                    path.AddEllipse(this.p1.X, this.p1.Y, -Math.Min(Math.Abs(this.p2.X - this.p1.X), Math.Abs(this.p2.Y - this.p1.Y)),
                        -Math.Min(Math.Abs(this.p2.X - this.p1.X), Math.Abs(this.p2.Y - this.p1.Y)));
                if (this.p2.X > this.p1.X && this.p2.Y > this.p1.Y) // xuống phải
                    path.AddEllipse(this.p1.X, this.p1.Y, Math.Min(Math.Abs(this.p2.X - this.p1.X), Math.Abs(this.p2.Y - this.p1.Y)),
                            Math.Min(Math.Abs(this.p2.X - this.p1.X), Math.Abs(this.p2.Y - this.p1.Y)));
                if (this.p2.X > this.p1.X && this.p2.Y < this.p1.Y) // lên phải
                    path.AddEllipse(this.p1.X, this.p1.Y, Math.Min(Math.Abs(this.p2.X - this.p1.X), Math.Abs(this.p2.Y - this.p1.Y)),
                        -Math.Min(Math.Abs(this.p2.X - this.p1.X), Math.Abs(this.p2.Y - this.p1.Y)));
                if (this.p2.X < this.p1.X && this.p2.Y > this.p1.Y) // xuống trái
                    path.AddEllipse(this.p1.X, this.p1.Y, -Math.Min(Math.Abs(this.p2.X - this.p1.X), Math.Abs(this.p2.Y - this.p1.Y)),
                        Math.Min(Math.Abs(this.p2.X - this.p1.X), Math.Abs(this.p2.Y - this.p1.Y)));

                //path.AddEllipse(p1.X, p1.Y, Math.Min(p2.X - p1.X, p2.Y - p1.Y), Math.Min(p2.X - p1.X, p2.Y - p1.Y));
        ...
        protected override GraphicsPath GraphicsPathPoint
        {
            ...
            {
                GraphicsPath path = new GraphicsPath();
                // path.AddEllipse(new System.Drawing.Rectangle(p1.X-4, p1.Y-4, 8, 8));
